...
local function wait_until_size(conn, s)
    while conn.buf_size < s do
        conn.wait_more_data = coroutine.running()
        print("wait "..s)
        skynet.wait()

        if conn.buf_size == 0 then -- error
            return 
        end
    end
    conn.wait_more_data = nil
    return true
end

function _M.handle_frame(conn, sock, max_payload_len, force_masking)
    assert(conn)
    if not wait_until_size(conn, 2) then
        print(1)
        return nil, nil, "wait data failed, wait size = 2"
    end
    ...
    local force_masking = conn.ctx.send_masked
    local max_payload_len = conn.ctx.max_payload_len
    ...
    if payload_len == 126 then
        if not wait_until_size(conn, 4) then
            print(6)
            return nil, nil, "wait data failed, wait size = 4"
        end
        local data = string.sub(conn.buffer,3,4)

        payload_len = ((byte(data, 1) << 8) | byte(data, 2))

    elseif payload_len == 127 then
        if not wait_until_size(conn, 12) then
            print(7)
            return nil, nil, "wait data failed, wait size = 12"
        end
        local data = string.sub(conn.buffer, 5, 12)
    ...
    if rest > 0 then
        print(type(rest))
        if not wait_until_size(conn, 12 + rest) then
            print(13)
            return nil, nil, "wait data failed, wait size = "..(12 + rest)
        end
        data = string.sub(conn.buffer, 12, 12+rest)
    else
        data = ""
    end
