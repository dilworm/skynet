...
static bool
ismarked(lua_State *dL, const void *p, const char* desc) {
...
static void calc_child_cnt(lua_State* L, lua_State* dL, int tid, const char* typename) {
    lua_pushnil(dL);
    while(lua_next(dL, tid) != 0) {
        if (!lua_istable(dL, -1)) {
            lua_pop(dL, 1); // 保留key 用于next
            continue;
        }
        // 遍历所有的父结点, 并给每个父结点的子结点数加1
        lua_pushnil(dL); 
        while (lua_next(dL, -2)!= 0) { // [pN, descN]
            const void* parent = lua_topointer(dL, -2); 
            lua_rawgetp(L, -1, parent);
            if (lua_isnil(L, -1)) {
                lua_rawgetp(dL, DESC, parent);
                const char* parent_desc = lua_tostring(dL, -1);
                lua_pop(L, 1); 
                lua_newtable(L);
                lua_pushstring(L, "desc");
                lua_pushstring(L, parent_desc);
                lua_rawset(L, -3);
...
static void gen_child_cnt(lua_State* L, lua_State* dL, int cnt) {
    int count = 0;
	count += count_table(dL, TABLE);
	count += count_table(dL, FUNCTION);
	count += count_table(dL, USERDATA);
	count += count_table(dL, THREAD);
	lua_createtable(L, 0, count);
    calc_child_cnt(L, dL, TABLE, "table");
    calc_child_cnt(L, dL, USERDATA, "userdata");
    calc_child_cnt(L, dL, FUNCTION, "function");
    calc_child_cnt(L, dL, THREAD, "thread");
}

static int 
child_count(lua_State* L) {
	int i;
	lua_State *dL = luaL_newstate();
	for (i=0;i<7;i++) {
		lua_newtable(dL);
